<?php
    ...
    $path = array(
			...
			"productdetail" => "include/productdetail.php",
    ...
    function getCurrentUser(){
        if(isset($_SESSION['_user_rakugo_id']))
            $user_id = $_SESSION['_user_rakugo_id'];
        else $user_id = 0;
        return 1;//$user_id;
    }
    ...
    function checkUser($id, $ps){
        return getValue("SELECT performer_id FROM tbl_performer WHERE per_password = '" . $ps . "' AND per_email = '" . $id . "'");
    }
    ...
    function getUserType(){
        $user_id = getCurrentUser();
        if($user_id != 0){
            $user_type = getValue("SELECT user_access_type FROM ".DB_PREFIX."user WHERE user_id = " . $user_id);
            if($user_type == 'admin') return ADMINISTRATOR;
            else if($user_type == 'owner') return OWNER;
        }
    }
